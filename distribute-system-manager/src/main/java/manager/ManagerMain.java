...
public class ManagerMain {
    ...
    public static void main(String[] args) {
        // parse args
        if (args.length < 3) {
            System.out.println("java -jar ManagerMain servers cmd subCmd [args]");
            System.exit(1);
        }
        ...
        Validate.isTrue(subCmd.equals("get")
                || subCmd.equals("add")
                || subCmd.equals("del"));
        RaftClientService client = new RaftClusterServiceProxy(servers);
        ...
        ((RaftClusterServiceProxy) client).stop();
    }
