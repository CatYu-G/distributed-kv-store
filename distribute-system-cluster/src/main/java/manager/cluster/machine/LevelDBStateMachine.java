package manager.cluster.machine;
...
import manager.core.raftimpl.raft.storage.SegmentedLog;
        ...
        try {
            ...
            for (long index = raftNode.getSnapshot().getMetaData().getLastIncludedIndex() + 1;
                ...
                if (entry.getType() == RaftProto.EntryType.ENTRY_TYPE_DATA) {
                    ClusterProto.SetRequest request = ClusterProto.SetRequest.parseFrom(entry.getData().toByteArray());
